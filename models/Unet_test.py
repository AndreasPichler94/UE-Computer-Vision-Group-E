...
class UNet2(nn.Module):
    def __init__(self, in_channels, num_classes, num_frames):
        ...
        # Return output 2D convolution
        # Note: in_channels here should be 64, not the original in_channels
        self.out = nn.Conv2d(in_channels=64, out_channels=num_classes, kernel_size=1)
...
if __name__ == "__main__":
    input_image = torch.rand((1, 12, 512, 512))  # Assuming 12 frames with 3 channels each
    #num_frames = 12  # Assuming a focal stack with 12 frames
    #input_image = torch.rand((1, 1, 512, 512))
    num_frames = 12
    model = UNet(1, 1, num_frames)
    #model = MiniUNet(1, 1, num_frames)
    ...
    # Create a path to the folder one level above and then to "data/train/"
    #folder_one_level_above = os.path.join(script_directory, "..", "data", "train")

    #data_loader = _get_dataloader(folder_one_level_above, focal_height=0, image_resolution=(128, 128), batch_size=1)
